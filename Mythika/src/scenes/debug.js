const debugScene = Scene.create({
  ...
  buildLines: function() {
    ...
    if (s.player) {
      ...
      this.data.lines.push('Weapon: ' + s.player.weaponEquipped + ' Lv.' + s.player.weaponLvl);
      this.data.lines.push('Armor: ' + s.player.armorEquipped + ' Lv.' + s.player.armorLvl);
      this.data.lines.push('Accessory: ' + s.player.accessoryEquipped + ' Lv.' + s.player.accessoryLvl);
    } else {
