...
(function() {
  ...
  // Hero Moment - for key scenes (Ashram, Journey, Combat, CharacterCreate, Cultivation, Punarjanma, Alchemy)
  // Matches CULTIVATION_SCENE_DESIGN.md spec: asymmetric layout, generous spacing, noise overlay
  Scene.HeroMoment = function(opts) {
    ...
    let ctaBtn = null;
    let enterAnim = 0;
    let enterComplete = reduceMotion;
    
    // Create the CTA button once (skip when no label — header-only use)
    if (!ctaBtn && ctaLabel) {
      ctaBtn = UI.MagneticBtn(0, 0, 240, 52, ctaLabel); // x,y will be set in render
      ctaBtn.onClick = ctaAction;
      ctaBtn.setTrailingIcon('sparkle');
    }
    
    return {
      // Expose the button so scenes can add it to their button array for input handling
      getCtaButton: function() { return ctaBtn; },
      
      render: function(ctx) {
        const cx = G.W/2;
        let y = reduceMotion ? 60 : 40; // Reduced motion: less top padding
        
        // Enter animation
        if (!enterComplete) {
          enterAnim = Math.min(1, enterAnim + 1/60 / 0.6); // 600ms
          const ease = 1 - Math.pow(1 - enterAnim, 3);
          ctx.save();
        ...
        if (eyebrow) {
          ...
          R.textCenter(ctx, eyebrow, cx, y + 16, accent || R.colors.gold, R.fonts.eyebrow || R.fonts.sm);
          y += 40;
        }
        ...
        // Subtext (max 60ch, 20 words) - using textSecondary color
        R.textCenter(ctx, subtitle, cx, y, R.colors.textSecondary, R.fonts.md);
        y += 48;
        
        // Primary CTA - MagneticBtn with gold styling, trailing sparkle icon
        // (skipped for header-only use when no ctaLabel was given)
        if (ctaBtn) {
          ctaBtn.x = cx - 120;
          ctaBtn.y = y;
          ctaBtn.render(ctx);
        ...
        if (!enterComplete) {
          if (enterAnim >= 1) enterComplete = true;
          ctx.restore();
        }
      }
    };
